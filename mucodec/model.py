...
# Optional fairseq import - only needed for encoding audio to codes
def load_model(model_dir, checkpoint_dir):
    """Load Fairseq SSL model (requires fairseq package)"""
    try:
        import fairseq
        ...
        raise ImportError(
            "fairseq is required for encoding audio. Install with: pip install fairseq\n"
            "Note: fairseq is NOT required for decoding audio tokens."
        )
...
class PromptCondAudioDiffusion(nn.Module):
    def __init__(
        ...
        self._muencoder_dir = "muq_dev/muq_fairseq"
        self._muencoder_ckpt = "muq_dev/muq.pt"

        # Lazy load muencoder - only needed for encoding, not decoding
        self._muencoder = None
    ...
    @property
    def muencoder(self):
        """Lazy-load muencoder only when needed (for encoding audio to codes)."""
        if self._muencoder is None:
            self._muencoder = load_model(
                model_dir=os.path.abspath(self._muencoder_dir),
                checkpoint_dir=os.path.abspath(self._muencoder_ckpt),
            )
            for v in self._muencoder.parameters():
                v.requires_grad = False
        return self._muencoder
