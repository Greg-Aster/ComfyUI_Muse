...
class MuCodec:
    ...
    def code2sound(self, codes, prompt=None, duration=40.96, guidance_scale=1.5, num_steps=20, disable_progress=False):
        # Ensure model is on GPU (may have been offloaded to CPU in previous run)
        if next(self.model.parameters()).device.type == 'cpu':
            print("[MuCodec] Moving diffusion model back to GPU...")
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            for sinx in range(0, codes.shape[-1]-hop_samples, hop_samples):
                ...
                if(sinx == 0):
                    ...
                    # Move to CPU immediately to free GPU memory for next iteration
                    latent_list.append(latents.cpu())
                else:
                    # Get previous latent back to GPU just for context extraction
                    prev_latent_gpu = latent_list[-1].to(self.device)
                    true_latent = prev_latent_gpu[:,:,-ovlp_frames:,:]
                    del prev_latent_gpu
        ...
        # Convert to float32 while keeping on CPU
        ...
        # Clear diffusion model from GPU before VAE decode to prevent OOM
        # The diffusion model is no longer needed, only VAE remains
        import gc

        # Move the diffusion model to CPU - it's not needed for VAE decode
        # This frees ~3-4GB of VRAM
        print("[MuCodec] Moving diffusion model to CPU for VAE decode...")
        self.model.cpu()
        ...
        if torch.cuda.is_available():
            ...
            print(f"[MuCodec] VRAM after diffusion offload: {torch.cuda.memory_allocated() / 1024**3:.2f}GB")

        with torch.no_grad():
            output = None
            for i in range(len(latent_list)):
                # Move latent from CPU to GPU just for this decode step
                latent = latent_list[i].to(self.device)
                # Clear the reference from list to free CPU memory
                latent_list[i] = None
                ...
                # Free latent memory immediately
                del latent
                torch.cuda.empty_cache()

                cur_output = self.vae.decode_to_waveform(mel)

                # Free mel memory immediately
                del mel
                torch.cuda.empty_cache()
                ...
                if output is None:
                    ...
                    # Free cur_output after merging into output
                    del cur_output
        ...
        # Move diffusion model back to GPU for next run
        self.model.to(self.device)
